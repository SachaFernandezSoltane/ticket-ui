...
@Component({
  selector: 'app-view-ticket-users',
  standalone: true,
  imports: [NavbarComponent, MatCardModule, MatDividerModule, MatButtonModule, MatProgressBarModule, NgFor],
  templateUrl: './view-ticket-users.component.html',
  styleUrl: './view-ticket-users.component.scss'
})
export class ViewTicketUsersComponent {
  ...
  fillData(): void {
    this.ticketService.getAllTickets().subscribe((tickets: any[]) => {
      tickets.forEach((element: any) => {
        ...
        this.ticketsToDisplay.push(newT);
      });
    }, (error: any) => {
